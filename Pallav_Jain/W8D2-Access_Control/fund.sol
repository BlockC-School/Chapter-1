...
contract CroudFunding {
    ...
    function voteRequest(uint256 _requestNumber) public{
        require(Contributers[msg.sender] > 0, "You must be a contributed");

        Request storage newvote = requests[_requestNumber];
        require(newvote.voters[msg.sender] == false, "You have already voted");
        newvote.voters[msg.sender] = true;
        newvote.noOfVoters +=1;
    }
    ...
    function makePayment(uint256 _requestNumber) public OnlyAdmin {
        ...
        require(newPayment.noOfVoters > NoOfCountributers/2, "Majorty Does not support");
